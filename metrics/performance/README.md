...
* SQL queries per data row
* Throughput (files per min)
* Total DB time


## Success Criteria

* Reduce median query count to < 1 query per row (including processing events)
* Reduce total DB time by at least 50%
* Preserve correctness checks at 100 percent pass rate

## Results
* Median query count reduced down to 0.69 per row
* Total DB time reduced by 80.2%
* Correctness checks preserved at 100%: 24 of of 24 invalid files and 6 out of 6 valid files were correctly identified and processed by the service. For rows with defects, recall was 99.5%, precision was 100.0% with 49,896 rows ingested. 
...
* Quick protocol (see details in 'Run protocol'):
  ...
  * Run each fixed CSV file 5 times. For each run replicate:
    ...
    * Perform correctness checks: no errors, status "COMPLETED" for the ingestion_id`,
