...
v1.1.0 adds a `trend_direction` value, "improved", so that an analyte which was
out of range in its most recent prior result but is now within range can be
surfaced as its own finding instead of silently folding into "normal, no
comment". The output schema (ai_annotation_content_v1_0_0) is unchanged —
`trend_direction` is a bare `str` there, so no content-version bump is needed.
"""
