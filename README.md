...
#### Historic lab results
For the current demo, historical lab results are retrieved directly via SQL 
query from the Observation table and injected into the LLM prompt alongside the
 new report and retrieved guideline chunks. This is intentional: with a 
 pre-loaded, bounded dataset, direct SQL is precise, staleness-free, and avoids
  the complexity of summarization pipelines.
In production, this approach breaks down as patient volume and observation
history grow — prompt injection of raw rows becomes token-prohibitive and query
 latency increases. A tiered design would be more appropriate: recent results 
 (last 3–5 per analyte) continue to be injected directly via SQL for precision;
  older history is compressed into prose TREND_SUMMARY chunks generated by a 
  background job and stored in the vector store for semantic retrieval; and 
  long-term statistical context is precomputed into a structured 
  patient_analyte_summary table and injected on demand.
