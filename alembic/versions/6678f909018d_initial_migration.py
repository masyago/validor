...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Enum(
                ...
                "FHIR_JSON_GENERATION_SUCCEEDED",
                "FHIR_JSON_GENERATION_FAILED",
                "FHIR_JSON_RESOURCE_FAILED",
        ...
        sa.Column(
            "severity",
            sa.Enum(
                "INFO", "WARN", "ERROR", name="processing_event_severity_enum"
            ),
            server_default=sa.text("'INFO'::processing_event_severity_enum"),
            nullable=False,
        ),
    ...
    op.create_index(
        ...
        "processing_event",
        ["ingestion_id", "occurred_at"],
        unique=False,
    )
    op.create_index(
        "ix_processing_event_target_type_id",
        "processing_event",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ux_processing_event_dedupe",
        table_name="processing_event",
        postgresql_where=sa.text("dedupe_key IS NOT NULL"),
    )
    op.drop_index(
        "ix_processing_event_target_type_id", table_name="processing_event"
    )
    op.drop_index(
        "ix_processing_event_ingestion_occurred_at",
        table_name="processing_event",
